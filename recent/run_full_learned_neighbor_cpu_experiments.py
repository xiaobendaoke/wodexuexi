...
def build_stage_env(base_env: dict[str, str], workers: int) -> dict[str, str]:
    stage_env = base_env.copy()
    torch_threads = max(1, CPU_COUNT // max(1, workers))
    stage_env.update(
        {
...
def run_tasks(tasks: list[Task], state_dir: Path, max_workers: int, retries: int, env: dict[str, str]) -> None:
    ...
    stage_env = build_stage_env(env, workers)
    print(
        f"[run] {len(pending)} task(s), workers={workers}, "
        f"threads_per_task={stage_env['OMP_NUM_THREADS']}, "
        f"logical_cpus={CPU_COUNT}"
    )
    failures: list[dict[str, Any]] = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        future_to_task = {executor.submit(run_task, task, state_dir, retries, stage_env): task for task in pending}
...
def make_smoke_tasks(run_dir: Path) -> list[Task]:
    smoke_done = run_dir / "state" / "smoke.ok"
    neighbor_done = run_dir / "state" / "learned_neighbor_mask.ok"
    neighbor_check_code = r"""
import pathlib
import sys
...
assert config.OFFLOAD_NUM_ACTIONS == config.OFFLOAD_ACTION_COOP_BASE + config.NUM_UAVS
env = Env()
env.reset()
obs, masks = env.get_offloading_obs_and_masks()
assert masks.shape == (config.NUM_UAVS, config.MAX_OFFLOAD_REQUESTS_PER_UAV, config.OFFLOAD_NUM_ACTIONS), masks.shape
bad = []
valid_coops = 0
for uav_idx, uav in enumerate(env.uavs):
    neighbor_ids = {neighbor.id for neighbor in uav.neighbors}
    for req_idx in range(config.MAX_OFFLOAD_REQUESTS_PER_UAV):
        for action_idx in range(config.OFFLOAD_ACTION_COOP_BASE, config.OFFLOAD_NUM_ACTIONS):
            target_id = action_idx - config.OFFLOAD_ACTION_COOP_BASE
            if masks[uav_idx, req_idx, action_idx] > 0.0:
                valid_coops += 1
                if target_id == uav_idx or target_id not in neighbor_ids:
                    bad.append((uav_idx, req_idx, action_idx, target_id, sorted(neighbor_ids)))
assert not bad, bad[:5]
model = OffloadMAPPO(
    "constrained_attention_offload_mappo",
    config.NUM_UAVS,
    config.OFFLOAD_OBS_DIM_SINGLE,
    config.OFFLOAD_NUM_ACTIONS,
    "cpu",
)
actions, log_probs, values = model.get_action_and_value(obs, masks=masks, exploration=True)
for uav_idx in range(config.NUM_UAVS):
    for req_idx in range(config.MAX_OFFLOAD_REQUESTS_PER_UAV):
        if masks[uav_idx, req_idx].sum() > 0.0:
            assert masks[uav_idx, req_idx, actions[uav_idx, req_idx]] > 0.0
print(
    {
        "mask_shape": tuple(masks.shape),
        "valid_coop_actions": valid_coops,
        "max_sampled_action": int(actions.max()),
        "log_prob_shape": tuple(log_probs.shape),
...
"""
    return [
        Task(
            ...
            done_paths=(smoke_done,),
            log_path=run_dir / "logs" / "smoke_offloading_consistency.log",
        ),
        Task(
            name="smoke_learned_neighbor_mask_and_actor",
            cmd=[sys.executable, "-c", neighbor_check_code, str(neighbor_done)],
            done_paths=(neighbor_done,),
            log_path=run_dir / "logs" / "smoke_learned_neighbor_mask_and_actor.log",
        ),
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run full CPU-parallel learned-neighbor UAV-MEC experiments with resumable task state. "
            "Re-run the same command after interruption; completed tasks are skipped."
        )
    )
    parser.add_argument("--run_name", default=time.strftime("learned_neighbor_full_%Y%m%d_%H%M%S"))
    ...
    parser.add_argument("--max_workers", type=int, default=CPU_COUNT, help="Max parallel tasks. Default uses all logical CPU cores.")
...
def main() -> None:
    ...
    metadata = {
        ...
        "experiment_family": "learned_neighbor_e2e",
        "suite": args.suite,
        ...
        "cpu_count": CPU_COUNT,
        "threading": "stage-aware: threads_per_task=floor(cpu_count/workers)",
        "action_space": "0=local, 1=MBS, 2+i=cooperative UAV i",
