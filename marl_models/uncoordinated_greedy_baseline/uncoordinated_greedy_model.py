"""
中文注释说明：marl_models/uncoordinated_greedy_baseline/uncoordinated_greedy_model.py

文件作用：
    实现无协调贪心基线模型的模型封装，负责动作选择、经验存储、网络更新、模型保存和加载。
...
关键变量与对象：
    - UncoordinatedGreedyModel: 核心类，封装本模块中的主要状态和行为。
...
# 类 UncoordinatedGreedyModel，继承自 MARLModel：核心类，封装本模块中的主要状态和行为。
