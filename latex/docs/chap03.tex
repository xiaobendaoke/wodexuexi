...
\end{equation}
其中 $W_J$、$W_L$、$W_E$、$W_O$、$W_{DSR}$ 和 $W_M$ 分别表示公平性、时延、能耗、离线率、截止时间满足率和 MBS 负载对应的奖励权重。主配置中取 $W_M=0.5$，对应实现中的轻量 MBS 负载惩罚项。当 UAV 发生碰撞或越界时，环境额外施加约束惩罚；为稳定 MARL 训练过程中的数值范围，可对最终奖励进行缩放处理。
...
其中 $g_i^{self}$ 包含 UAV 归一化位置、当前服务请求队列比例、邻居数量比例和 UAV-MBS 回传速率；$G_i^{req}$ 由最多 30 个 request slot 组成。每个有效请求 slot 包含请求大小、请求编号、deadline、priority、本地缓存命中、协作可用性、本地队列长度、local/cooperative/MBS 时延相对 deadline 的截断比例、最佳邻居计算份额以及 UE-UAV 相对位置等特征。
...
其中 $\eta_{\lambda} = 0.1$ 为 Lagrange 学习率，$\lambda_{max} = 10.0$ 为上界，$\bar{v}$ 为 rollout 平均约束违反量。该机制通过提高违反约束时的惩罚权重引导策略减少 DSR 不足和 MBS 负载超限，但属于训练目标中的软约束，并不构成严格可行性保证。
...
实验区域为 $700$m $\times$ 700m，系统包含 $N=5$ 架 UAV、$M=100$ 个 UE 和 $1$ 个 MBS。每个 episode 包含 $T=1000$ 个 time slots，每个 time slot 时长为 $\tau = 1$s。UAV 飞行高度 $H=100$m，最大速度 $v_{max}=15$m/s，覆盖半径 $R_c=100$m，感知范围 $460$m，最小 UAV 间距 $200$m；MBS 位于区域中心附近并具有固定高度。UE 初始位置采用热点分布以形成非均匀业务负载，并在 episode 中按照移动模型更新位置。系统包含 25 类服务和 50 类内容文件，服务 deadline 在 $[0.65, 2.10]$s 范围内生成。每架 UAV 配置有限缓存容量，UAV 计算能力在 $[40, 90] \times 10^9$ cycles/s 之间随机生成，MBS 固定算力 $f_{mbs} = 200 \times 10^9$ cycles/s。UE-UAV、UAV-UAV 和 UAV-MBS 链路分别使用接入带宽、协作带宽和回传带宽建模。
