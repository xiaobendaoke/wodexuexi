...
本文在 Python/PyTorch 仿真平台上验证所提算法。系统环境包含多 UAV、UE 与 MBS 三类节点，并同时模拟通信链路、计算卸载、缓存获取、UE 电池演化和无线能量传输过程。主要仿真环境参数如表 \ref{tab:simulation-params} 所示。
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{ll}
        ...
        UAV 计算能力 & $[40,90]\times10^9\,\mathrm{cycles/s}$ \\
...
本文算法训练参数如表 \ref{tab:algorithm-params} 所示。正式主实验采用 3 个 training seeds 和 10 个 workload seeds；每个 workload seed 运行 6 个 episodes，并以 (training seed, workload seed) 的 episode mean 作为统计单元，因此每个学习式策略共有 $N=30$ 个统计样本。本文报告 mean、std、95\% CI、paired delta、paired t-test 和 Wilcoxon 检验。
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{ll}
        ...
        \hline
        训练随机种子 & $42,84,126$ \\
        workload 随机种子 & $42,84,126,168,210,252,294,336,378,420$ \\
        训练 episodes / 每个 workload 评估 episodes & $200$ / $6$ \\
        MLP 隐层维度 & $128$ \\
        attention 隐层维度 / 注意力头数 & $64$ / $8$ \\
        每 UAV 最大 request slot 数 $K$ & $30$ \\
        优化器 / 学习率 & Adam / $3\times10^{-4}$ \\
        PPO clip 系数 $\epsilon$ & $0.2$ \\
        折扣因子 $\gamma$ / GAE 系数 $\lambda$ & $0.99$ / $0.95$ \\
        rollout 长度 / PPO epochs / batch size & $1000$ / $8$ / $256$ \\
        entropy coefficient & $0.005$ \\
        系统 reward 权重 & $W_J=1.0$, $W_L=1.0$, $W_E=0.5$, $W_O=5.0$, $W_{DSR}=1.0$, $W_M=0.5$ \\
        下层 reward 权重 & $w_{succ}=0.4$, $w_{coop}=0.08$, $w_{dead}=3.0$, $w_{lat}=4.0$, $w_{en}=1.2$, $w_{mbs}=0.35$ \\
        Lagrange 约束目标 & $\tau_{dsr}=0.18$, $\tau_{mbs}=0.03$ \\
        Lagrange 学习率 / 乘子上界 & $0.1$ / $10.0$ \\
        MBS mask deadline clip 阈值 & $10$ \\
        cooperative mask 阈值 & $\theta_{deadline}=1.50$, $\theta_{latency}=1.35$, $\theta_{share}=0.25$ \\
        \hline
    \end{tabular}}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{DSR}：deadline satisfaction rate，所有生成服务请求中被成功分配且满足 deadline 的比例
    \item \textbf{offline\_rate ($O$)}：电池低于 $B_{low}$ 的 UE 比例
    \item \textbf{Offloading ratios}：local/cooperative/MBS 卸载比例，采用已处理服务请求作为分母，用于描述实际卸载动作分布
    \item \textbf{MBS load ratio}：卸载至 MBS 的服务请求数占全部生成服务请求数的比例，用于衡量系统对 MBS 的总体依赖程度；其分母不同于 MBS offloading ratio
\end{itemize}
...
完整双层 MARL 的主要收益集中在系统奖励、能耗、公平性和在线率四个维度。相对基础参考，reward 提升 4482.2（$p = 1.0\times10^{-32}$），UAV 侧能耗降低 56.64M（$p = 1.7\times10^{-19}$），公平性提升 0.162（$p = 1.7\times10^{-11}$），offline rate 降至 0.00\%。这一结果说明，双层框架能够通过轨迹协同和请求级卸载重分配降低高能耗服务模式，并使服务覆盖在 UE 间更加均衡。在 deadline satisfaction 方面，能耗优先配置下完整方法的 DSR 为 0.2083，低于基础参考的 0.2734（$\Delta=-0.0651$, $p=2.4\times10^{-11}$）。因此，该主配置更适合被解释为能耗、公平性和在线稳定性优先的工作点；后续 DSR-priority 配置进一步验证了该框架可通过奖励权重和约束目标调节服务质量偏好。
...
在该补充实验中，vanilla\_mappo 在 reward、latency 和 fairness 上取得更高数值；attention 相对 vanilla 的 latency 增加约 293458（paired t-test $p=0.0085$），reward 降低约 759.5（$p=0.0889$）。另一方面，attention-MAPPO 的 energy 更低（约 -5.57M，$p=0.102$），offline rate 更低，MBS load ratio 也显著更低（0.99\% vs 1.41\%，$p=0.0155$）。这说明 attention 机制在当前设置下主要改变的是轨迹协同和负载分配偏好，而不是在所有单项指标上同步提升。
...
卸载分布的变化解释了能耗收益的来源。Heuristic 基线主要依赖 local 和 cooperative 两类执行方式，MBS offloading ratio 仅为 2.6--2.7\%；引入下层 MAPPO 后，策略开始更主动地在 local、cooperative 和 MBS 之间分配请求，其中 MBS offloading ratio 提升至 11--17\%。完整双层方法没有选择最激进的 MBS 使用方式，而是在 local 46.4\%、cooperative 42.4\%、MBS offloading ratio 11.1\% 的分布下将 offline rate 降至 0.00\%。这说明下层策略的作用不是单纯增加某一种卸载模式，而是在能耗、可服务性和 MBS 约束之间重新分配请求。
...
图 \ref{fig:baseline-matrix-overview} 表明，本文方法的优势主要集中在系统公平性、在线稳定性和请求级服务质量。Proposed 的 final fairness 为 0.9290，在所有学习式方法中最高；相对 IPPO、Vanilla MAPPO 和 Joint MAPPO 分别提升约 3.84\%、0.96\% 和 107.14\%。同时，Proposed 的 final offline rate 和 step-mean offline rate 均为 0，显著低于 IPPO、Vanilla MAPPO、Joint MAPPO 和 Uniform，说明所提双层策略能够更稳定地避免低电量或服务失衡导致的离线现象。

在 deadline satisfaction 方面，Proposed 的 request-weighted DSR 为 0.1934，相比 IPPO、Vanilla MAPPO、Joint MAPPO 和 Random 分别提升约 10.09\%、7.03\%、64.53\% 和 27.71\%。Uniform 的 request-weighted DSR 为 0.2273，体现出均匀卸载策略在该指标上的激进服务倾向；相比之下，Proposed 以更低能耗、更高公平性和更低 MBS 负载形成更稳健的综合工作点。能耗方面，Proposed 相比 Vanilla MAPPO、Joint MAPPO、Random 和 Uniform 分别降低约 4.98\%、15.06\%、24.04\% 和 40.82\%，进一步说明所提方法更适合面向多目标约束场景，而非单一指标最大化。
...
图 \ref{fig:baseline-matrix-mechanism} 进一步解释了上述优势的来源。Proposed 的 processed-denominator MBS offloading ratio 为 14.16\%，低于 IPPO 的 18.97\%、Joint MAPPO 的 22.64\% 和 Random 的 30.36\%；generated-denominator MBS load ratio 为 6.63\%，相比 IPPO 和 Random 分别降低约 32.62\% 和 37.06\%。这说明本文方法并非通过简单增加 MBS 依赖来提高服务完成质量，而是在 local、cooperative 和 MBS 之间形成更受约束的负载分配。另一方面，Proposed 的 processed request ratio 低于 IPPO 和 Vanilla MAPPO，但 deadline-satisfied-per-processed ratio 分别高出约 22.84\% 和 25.39\%，表明其更倾向于处理更有把握满足 deadline 的请求。综合来看，扩展 baseline 矩阵支持本文方法的核心定位：所提方法追求的是公平、可靠和低 MBS 依赖的多目标折中。
...
从图 \ref{fig:lr-convergence} 可以看出，四组学习率对应的平均累计奖励均在训练前期快速上升，并在后期逐渐趋于稳定，说明所提双层策略能够从环境交互中学习有效的轨迹与卸载行为。学习率为 $1\times10^{-4}$ 时，累计平均奖励曲线上升较为平稳，但策略更新速度相对较慢，末 20 回合单回合平均奖励为 -1411.9；学习率为 $3\times10^{-4}$ 时，收敛水平与 $1\times10^{-4}$ 接近，末 20 回合单回合平均奖励为 -1425.6；学习率为 $5\times10^{-4}$ 时，训练后期奖励水平较高，末 20 回合单回合平均奖励达到 -1203.4；而学习率为 $1\times10^{-3}$ 时，奖励曲线波动较为明显，末 20 回合单回合平均奖励下降至 -2093.9。上述结果表明，过小的学习率会降低策略更新效率，过大的学习率则容易引入训练波动。需要说明的是，本节学习率敏感性实验固定 random seed 和 workload seed 为 42，主要用于分析学习率变化对训练稳定性和收敛趋势的影响，并不作为全局最优学习率的充分证明。综合考虑收敛稳定性、指标均衡性以及正式主实验设置的一致性，本文后续实验仍采用表 \ref{tab:algorithm-params} 中的默认学习率 $3\times10^{-4}$。
...
DSR-priority 配置下，完整方法的 DSR 从能耗优先配置下的 0.2083 提升至 0.2435，与 baseline 的差距从 -0.065 缩小至 -0.030（$p = 1.6\times10^{-6}$）。该结果表明，调整 DSR 奖励权重和 Lagrange 约束目标能够将策略从能耗优先工作点推向服务质量优先工作点。需要注意的是，DSR-priority 配置的 DSR 仍低于 heuristic baseline，因此该实验体现的是服务质量偏好的可调节性，而非对所有 QoS 指标的全面支配。与此同时，DSR-priority 配置下能耗为 83.04M，相比 baseline 的 114.70M 仍节省 27.6\%；cooperative ratio 达到 56.7\%，为所有学习式策略中最高。
